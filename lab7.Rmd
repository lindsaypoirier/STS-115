...
When dates are stored as column names and not as values in and of themselves, it can be very difficult to manipulate the data based on dates. For an explanation of why this is, see: https://r4ds.had.co.nz/tidy-data.html The code below transforms this data into a tidy format. In other words, using **gather()**, we create a new row per province/country for every date column in the dataset and the totals associated with that date. Since R automatically places an "X" character in front of numeric column names, we remove that X character and then we transform the values into a date-time format. If you'd like to know more about this, I highly recommend the chapter in R for Data Science above. 
...
```{r}
nyc311 <- read.csv("", stringsAsFactors = FALSE)
